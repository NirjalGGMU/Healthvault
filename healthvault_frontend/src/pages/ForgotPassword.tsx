...
import { PasswordResetRequestResponse } from '../types';

/**
 * Requests a password reset link. No mail provider is configured in this
 * project, so the backend logs the link server-side and (outside production
 * only) echoes it back here as `devResetLink` — shown directly on the page
...
const ForgotPassword = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setDevLink(null);
    try {
      const { data } = await api.post<PasswordResetRequestResponse>('/auth/forgot-password', {
        email: email.trim(),
      });
  ...
  return (
    <div className="page mx-auto flex min-h-[80vh] max-w-md flex-col justify-center px-4 py-8 sm:py-12">
      <div className="card">
        <h1 className="text-xl font-bold text-gray-900 dark:text-white sm:text-2xl">{t('forgotPassword.title')}</h1>
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{t('forgotPassword.subtitle')}</p>

        <form onSubmit={handleSubmit} className="mt-6 space-y-4" noValidate>
          <div>
            <label htmlFor="email" className="label">{t('forgotPassword.email')}</label>
            <input
              id="email"
              type="email"
        ...
        {devLink && (
          <div className="mt-4 rounded-lg border border-amber-300 bg-amber-50 p-3 text-xs text-amber-800 dark:border-amber-700 dark:bg-amber-950/40 dark:text-amber-300">
            <p className="font-semibold">{t('forgotPassword.devNote')}</p>
